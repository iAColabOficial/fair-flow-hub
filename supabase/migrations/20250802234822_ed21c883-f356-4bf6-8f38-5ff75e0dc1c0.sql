-- Fix the handle_new_user function to remove user_role variable reference
...
BEGIN
  ...
  DECLARE
    febic_role febic_role := COALESCE(NEW.raw_user_meta_data->>'role', 'autor')::febic_role;
    role_status text := CASE 
      WHEN febic_role IN ('autor', 'orientador') THEN 'ativo'
      ELSE 'pendente'
    END;
  BEGIN
...
-- Now clear all data and create admin user
...
-- Create admin user manually (since trigger creates regular profiles)
DO $$
DECLARE
