-- Primeiro, limpar CPFs duplicados mantendo apenas o primeiro registro
...
records_to_keep AS (
  SELECT p.id 
  FROM profiles p
  INNER JOIN duplicated_cpfs d ON p.cpf = d.cpf AND p.created_at = d.first_created
)
UPDATE profiles 
SET cpf = NULL 
WHERE cpf IS NOT NULL 
  AND cpf IN (SELECT cpf FROM duplicated_cpfs)
